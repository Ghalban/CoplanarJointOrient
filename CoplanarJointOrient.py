...
import CoplanarJointOrient.mayaUtil as mayaUtil
from CoplanarJointOrient.PlaneMode import AdvancedPlaneMode, Automatic3PointPlaneMode, AutomaticOrientedPlaneMode, \
    AxisAlignedPlaneMode
from CoplanarJointOrient.MayaUIValue import AxisValue, BoolValue, JointValue, NormalModeValue
import CoplanarJointOrient.mayaMathUtil as mayaMathUtil
...
def createDebugVisualizerPlane(plane, position=None, name=None):
    ...
    # TODO: for now this creates a plane that ends up affecting the scene history, so undo/redo actions end up 
    #  recreating the plane. Figure out how to draw this plane without this happening
    debugPlane = cmds.polyPlane(name=name if name else "debugPlane", width=100, height=100,
                                axis=[plane.normal().x, plane.normal().y, plane.normal().z], constructionHistory=False)
    # make plane unselectable and undeletable so users can't accidentally delete it and break things
    cmds.toggle(debugPlane, state=True,
                template=True)  
...
"""
Now that the UI did all the hard work of configuring the alignment plane, requiring like > 1000 lines of code, 
all the real work happens here in this one function
(Well technically a few outside util methods are called as well...)
"""


def coplanarizeJoints(chainEnd, chainRoot, plane, forwardAxis=mayaMathUtil.Axis(axis=0),
    ...
    while currJoint:
        ...
        if lastJoint is not None or (lastJoint is None and currChildren is None):
            ...
            # if no lastJoint, this is the last child in the joint chain so project its previous aim direction to a 
            # point on the plane and aim it in that direction to keep it approximately the same
            # there's probably a more mathematically correct way to do this but whatever...
            if lastJoint is None:
                deleteAimTarget = True
                aimLocation = mayaMathUtil.closestPointOnPlane(plane=plane,
                                                               point=oldPos + mayaMathUtil.eulerToDirectionVector(
                                                                   oldOrient, forwardAxis) * 10)
                aimTarget = cmds.spaceLocator()
                cmds.xform(aimTarget, worldSpace=True, absolute=True,
            ...
            tempAimConstraint = cmds.aimConstraint(aimTarget, currJoint, weight=1,
                                                   aimVector=[aimVector.x, aimVector.y, aimVector.z],
                                                   upVector=[turnVector.x, turnVector.y, turnVector.z],
                                                   worldUpVector=[plane.normal().x, plane.normal().y, plane.normal().z],
                                                   worldUpType="vector")
...
class CoplanarJointOrient(object):
    def __init__(self):
        # for the UI
        self.endJoints_instructions = None
        ...
        self.advancedPlaneMode = AdvancedPlaneMode.AdvancedPlaneMode(self)
        self.axisAlignedPlaneMode = AxisAlignedPlaneMode.AxisAlignedPlaneMode(self)
        self.automatic3PointPlaneMode = Automatic3PointPlaneMode.Automatic3PointPlaneMode(self)
        self.automaticOrientedPlaneMode = AutomaticOrientedPlaneMode.AutomaticOrientedPlaneMode(self)
    ...
    def setupUI(self):
        self.mainWindow = cmds.window(title="Coplanar Joint Orient Tool", sizeable=True, maximizeButton=False,
                                      minimizeButton=False, width=WINDOW_WIDTH, resizeToFitChildren=True)
        ...
        self.joint1 = JointValue.JointValue(label="Joint 1", parentUI=endJoints_layout)
        self.joint2 = JointValue.JointValue(label="Joint 2", parentUI=endJoints_layout)
        ...
        self.aimAxis = AxisValue.AxisValue(label="Aim Axis", parentUI=options_layout)
        self.turnAxis = AxisValue.AxisValue(label="Turn Axis", parentUI=options_layout)
        ...
        self.previewPlaneSetting = BoolValue.BoolValue(label="Preview Plane", parentUI=options_layout)
    ...
    def loadSettings(self):
        ...
        self.advancedPlaneMode.planeNormalMode.setValue(
            NormalModeValue.NormalModeValue.PLANE_NORMAL_MODE_VECTOR)
    ...
    def onAimAxisChanged(self, value):
        ...
        if self.turnAxis.value.axis == value.axis:
            self.turnAxis.setValue(
                mayaMathUtil.Axis(axis=(0 if value.axis != 0 else 1), negative=self.turnAxis.value.negative))

    def onTurnAxisChanged(self, value):
        # see if aim axis is same, and force it to change
        if self.aimAxis.value.axis == value.axis:
            self.aimAxis.setValue(
                mayaMathUtil.Axis(axis=(0 if value.axis != 0 else 1), negative=self.aimAxis.value.negative))
    ...
    def apply(self, unused):
        ...
        if self.chainEnd is not None and self.chainRoot is not None:
            coplanarizeJoints(chainEnd=self.chainEnd, chainRoot=self.chainRoot,
                              plane=self.currentPlaneMode.alignmentPlane, forwardAxis=self.aimAxis.value,
                              rotationAxis=self.turnAxis.value)
    ...
    def updatePreviewPlane(self):
        ...
        if self.previewPlaneSetting.value and self.currentPlaneMode is not None:
            self.previewPlane = createDebugVisualizerPlane(self.currentPlaneMode.alignmentPlane,
                                                           self.currentPlaneMode.alignmentPlanePreviewLocation)
        else:
            self.previewPlane = None
    ...
    def deletePreviewPlane(self):
        ...
        if self.previewPlane is not None:
            try:
                # It's possible a new scene was opened while this window is still open and the preview 
                # plane no longer exists
