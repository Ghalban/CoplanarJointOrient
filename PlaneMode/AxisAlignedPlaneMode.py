...
class AxisAlignedPlaneMode(CoplanarJointOrient.PlaneMode.PlaneMode.PlaneMode):
    ...
    def setupUI(self, parentUI):
        self.normalAxis = CoplanarJointOrient.MayaUIValue.AxisValue.AxisValue(label="Normal Direction",
                                                                              parentUI=parentUI)
        self.worldOffset = CoplanarJointOrient.MayaUIValue.PositionValue.PositionValue(label="World Offset",
                                                                                       parentUI=parentUI)

        self.normalAxis.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AxisAlignedPlaneMode.AxisAlignedPlaneMode.onAxisChanged, self)
        self.worldOffset.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AxisAlignedPlaneMode.AxisAlignedPlaneMode.onOffsetChanged, self)
    ...
    def updatePlane(self):
        ...
        self.alignmentPlane.setPlane(CoplanarJointOrient.mayaMathUtil.forwardVector(self.normalAxis.value),
                                     self.worldOffset.value[self.normalAxis.value.axis] * (
                                         1 if self.normalAxis.value.negative else -1))
