...
class AdvancedPlaneMode(CoplanarJointOrient.PlaneMode.PlaneMode.PlaneMode):
    ...
    def setupValues(self, parentUI):
        self.planePosition = CoplanarJointOrient.MayaUIValue.PositionValue.PositionValue(label="Position",
                                                                                         parentUI=parentUI)
        self.planePosition.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AdvancedPlaneMode.AdvancedPlaneMode.onPositionChange, self)

        self.planeNormalMode = CoplanarJointOrient.MayaUIValue.NormalModeValue.NormalModeValue(parentUI=parentUI)
        self.planeNormalMode.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AdvancedPlaneMode.AdvancedPlaneMode.onNormalModeChange, self)

        self.planeNormalVector = CoplanarJointOrient.MayaUIValue.DirectionVectorValue.DirectionVectorValue(
            label="Direction", parentUI=parentUI)
        self.planeNormalVector.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AdvancedPlaneMode.AdvancedPlaneMode.onNormalVectorChange, self)

        self.planeNormalPoint0 = CoplanarJointOrient.MayaUIValue.PositionValue.PositionValue(label="Point 1",
                                                                                             parentUI=parentUI)
        self.planeNormalPoint1 = CoplanarJointOrient.MayaUIValue.PositionValue.PositionValue(label="Point 2",
                                                                                             parentUI=parentUI)
        self.planeNormalPoint2 = CoplanarJointOrient.MayaUIValue.PositionValue.PositionValue(label="Point 3",
                                                                                             parentUI=parentUI)

        self.planeNormalPoint0.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AdvancedPlaneMode.AdvancedPlaneMode.onPoint01Change, self)
        self.planeNormalPoint1.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AdvancedPlaneMode.AdvancedPlaneMode.onPoint01Change, self)
        self.planeNormalPoint2.onChangeFunc = functools.partial(
            CoplanarJointOrient.PlaneMode.AdvancedPlaneMode.AdvancedPlaneMode.onPoint2Change, self)

    def updatePlane(self):
        planeNormal = om.MVector(om.MVector.kXaxisVector)

        if (
                self.planeNormalMode.value == CoplanarJointOrient.MayaUIValue.NormalModeValue.NormalModeValue.PLANE_NORMAL_MODE_VECTOR):
            planeNormal = self.planeNormalVector.value
        elif (
                self.planeNormalMode.value == CoplanarJointOrient.MayaUIValue.NormalModeValue.NormalModeValue.PLANE_NORMAL_MODE_3POINT):
            planeNormal = CoplanarJointOrient.mayaMathUtil.get3PointNormal(self.planeNormalPoint0.value,
                                                                           self.planeNormalPoint1.value,
                                                                           self.planeNormalPoint2.value)
        elif (
            ...
            planeNormal = CoplanarJointOrient.mayaMathUtil.get2PointNormal(self.planeNormalPoint0.value,
                                                                           self.planeNormalPoint1.value,
                                                                           self.planeNormalVector.value)
        else:
        ...
        if (
                self.planeNormalMode.value == CoplanarJointOrient.MayaUIValue.NormalModeValue.NormalModeValue.PLANE_NORMAL_MODE_3POINT or
                self.planeNormalMode.value == CoplanarJointOrient.MayaUIValue.NormalModeValue.NormalModeValue.PLANE_NORMAL_MODE_2POINT_VECTOR):
            if self.coplanarizer.chainRoot is not None and self.coplanarizer.chainEnd is not None:
                averageNormal = CoplanarJointOrient.mayaUtil.getAverageNodeDirectionVector(
                    CoplanarJointOrient.mayaUtil.getWholeParentChain(self.coplanarizer.chainRoot,
                                                                     self.coplanarizer.chainEnd),
                    self.coplanarizer.turnAxis.value)
